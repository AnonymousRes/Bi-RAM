...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    if condition:
        ...
        # 添加 dtype=object 强制允许不规则形状的数组保存
        safe_per_batch = {k: np.array(v, dtype=object) for k, v in per_batch.items()}
        np.savez(pred_filepath, **safe_per_batch)
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        if self.classification:
            ...
            probs = torch.nn.functional.softmax(
                predictions, dim=1)  # (total_samples, num_classes) est. prob. for each class and sample
