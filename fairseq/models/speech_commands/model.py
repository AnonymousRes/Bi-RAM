...
from fairseq.models.speech_commands.biram_scraw_encoder import BiRAMSCEncoder
...
class SCRawModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--k-times', type=int, metavar='N', help='expansion multiples of K_{comp}, V_{comp} matrices in Prefixformer')
...
@register_model_architecture('sc_raw', 'biram_sc_raw')
def biram_lra_sc(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'biram')

    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 64)
    args.encoder_hidden_dim = getattr(args, 'encoder_hidden_dim', 64)
