...
class LRAModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--k-times', type=int, metavar='N',
                            help='expansion multiples of K_{comp}, V_{comp} matrices in BiRAM')
        ...
        parser.add_argument('--layer-type', choices=['transformer', 'luna', 'lstm', 'flash', 'mega', 'biram'],)
...
class LRAEncoder(FairseqEncoder):
    ...
    def __init__(self, args, task):
        ...
        elif args.layer_type == 'biram':
            self.encoder = BiRAMLRAEncoder(
                tie_layer_weights=getattr(args, 'tie_layer_weights', False),
                padding_idx=padding_idx,
                vocab_size=vocab_size,
                num_encoder_layers=args.encoder_layers,
...
@register_model_architecture('lra', 'biram_lra_listop')
def biram_lra_listop(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'biram')
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
...
def luna_lra_imdb_architecture(args):
    args.layer_type = getattr(args, 'layer_type', 'luna')
    args.encoder_projection_length = getattr(args, 'encoder_projection_length', 64)
...
@register_model_architecture('lra', 'biram_lra_imdb')
def biram_lra_imdb(args):
    args.layer_type = getattr(args, 'layer_type', 'biram')
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.max_positions = getattr(args, 'max_positions', 4002)
    args.encoder_normalize_before = getattr(args, 'encoder_normalize_before', True)
...
@register_model_architecture('lra', 'biram_lra_aan')
def biram_lra_aan(args):
    args.layer_type = getattr(args, 'layer_type', 'biram')
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.max_positions = getattr(args, 'max_positions', 4002)
...
@register_model_architecture('lra', 'biram_lra_cifar10')
def biram_lra_cifar10(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'biram')
...
@register_model_architecture('lra', 'biram_lra_pf32')
def biram_lra_pf32(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.encoder_learned_pos = getattr(args, 'encoder_learned_pos', False)
    args.layer_type = getattr(args, 'layer_type', 'biram')
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
...
# def transformer_lra_pf128(args):
#     args.apply_bert_init = getattr(args, 'apply_bert_init', False)
#     args.layer_type = getattr(args, 'layer_type', 'transformer')
#     args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 128)
#     args.encoder_layers = getattr(args, 'encoder_layers', 1)
...
@register_model_architecture('lra', 'biram_lra_pf128')
def biram_lra_pf128(args):
    args.apply_bert_init = getattr(args, 'apply_bert_init', False)
    args.layer_type = getattr(args, 'layer_type', 'biram')
    args.encoder_layers = getattr(args, 'encoder_layers', 4)
    args.activation_fn = getattr(args, 'activation_fn', 'silu')
